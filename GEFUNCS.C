	...
	for (othusn = 0; othusn < nships; ++othusn) {
		...
		if (ingegame(othusn) && (wptr->where != 1 || ptr->phasrtype >= phatowrp) && ddistance < 100000.0) {
			...
			if (othusn != usrn
				...
				if (smallest(heading,deg) < (unsigned)(ptr->percent + PHABIAS)) {
...
void FUNC checktm(WARSHP *ptr, int usrn)
{
		...
		if (tptr->distance > 0) {
			...
			if (neutral(&ptr->coord) && tptr->distance < 5000) {
				...
				if (tptr->distance > 0) {	/* torp still here? */
					tptr->distance = (USHORT)(tptr->distance - torpsped);
					if (tptr->distance > 0)
						++track_cnt;
				}
			}
	...
	for (i = 0, mptr = ptr->lmissl; i < MAXMISSL; ++i, ++mptr) {
		...
		float menergy, mscale;
			...
			mscale = (float)sqrt(5000.0f / menergy);
			if (mscale > 2.0f)
				mscale = 2.0f;
			if (mscale < 0.5f)
				mscale = 0.5f;
			...
			if (neutral(&ptr->coord) && mptr->distance < 5000) {
				...
				if (mptr->distance > 0) {	/* missl still here? */
					mptr->distance = (USHORT)(mptr->distance - mstep);
					if (ptr->speed > 100000.0 || mptr->distance > 50000U - (int)(ptr->speed / 6.5) ||
						...
						mptr->distance = (USHORT)(mptr->distance + (int)(ptr->speed / 6.5));
						mptr->energy = (USHORT)(mptr->energy - 500);	/* decrease energy over time */
					}
					if (mptr->distance > 0)
						++track_cnt;
				}
...
void FUNC randamage(WARSHP *ptr, int usrn, double hitdam)
{
	...
	case 0: /* missiles */
		if (shipclass[ptr->shpclass].max_missl > 0) {
			if (dosys == 1 && ptr->mislcntl == 0) {
				ptr->mislcntl = (byte)(2 + r % (damcomb/3));
				prfmsg(RNDMISL);
				dosys = 2;
			}
	...
	case 1: /* torpedoes */
		if (shipclass[ptr->shpclass].max_torps > 0) {
			if (dosys == 1 && ptr->torpcntl == 0) {
				ptr->torpcntl = (byte)(2 + r % (damcomb/3));
				prfmsg(RNDTORP);
				dosys = 2;
			}
	...
	case 2: /* decoys */
		if (shipclass[ptr->shpclass].has_decoy > 0) {
			if (dosys == 1 && ptr->decload >= 0) {
				ptr->decload = (SHORT)(-2 - r % (damcomb/3));
				prfmsg(RNDDECY);
				dosys = 2;
			}
	...
	case 3: /* zippers */
		if (shipclass[ptr->shpclass].has_zip > 0) {
			if (dosys == 1 && ptr->zipload >= 0) {
				ptr->zipload = (SHORT)(-2 - r % (damcomb/3));
				prfmsg(RNDZIPR);
				dosys = 2;
			}
	...
	case 4: /* jammers */
		if (shipclass[ptr->shpclass].has_jam > 0) {
			if (dosys == 1 && ptr->jamload >= 0) {
				ptr->jamload = (SHORT)(-2 - r % (damcomb/3));
				prfmsg(RNDJAMR);
				dosys = 2;
			}
	...
	case 5: /* mines */
		if (shipclass[ptr->shpclass].has_mine > 0) {
			if (dosys == 1 && ptr->mineload >= 0) {
				ptr->mineload = (SHORT)(-2 - r % (damcomb/3));
				prfmsg(RNDMINE);
				dosys = 2;
			}
	...
	case 6: /* shields */
		if (shipclass[ptr->shpclass].max_shlds > 0 && ptr->shieldstat != SHIELDDM && dosys == 1) {
			prfmsg(SHDAMAG);
			ptr->shield = (SHORT)(-2 - r % (damcomb/3));
	...
	case 7: /* phasers */
		if (shipclass[ptr->shpclass].max_phasr > 0 && ptr->phasr >= 0 && dosys == 1) {
			prfmsg(RNDPHSR);
			ptr->phasr = (double)(-2 - r % (damcomb/3));
			dosys = 2;
		}
		break;
	...
	case 9: /* scanners */
		if (ptr->tactical == 0 && dosys == 1) {
			prfmsg(RNDTACT);
			ptr->tactical = (SHORT)(-2 - r % (damcomb/6));
			dosys = 2;
		}
		break;
...
void FUNC setsect(WARSHP *ptr)
{
	...
	pkey.xsect = (SHORT)xsect;
	pkey.ysect = (SHORT)ysect;
	pkey.plnum = 0;
}
...
int FUNC shieldhit(WARSHP *wptr, int dam)   /* 0% to 100% damage */
{
	...
	wptr->shield = (SHORT)(wptr->shield - knock);
	if (wptr->shield <=2 ) {
		wptr->shieldstat = SHIELDDM;
		wptr->shield = (SHORT)(wptr->shield - (knock * 3));
		return 1;
	}
...
void FUNC shieldchg(WARSHP *wptr, int usrn)
{
	...
	if (wptr->shield < maxcharge) {
		wptr->shield = (SHORT)(wptr->shield + (type * 3));
		if (wptr->shield >= maxcharge) {
			wptr->shield = (SHORT)maxcharge;
			prfmsg(SHLDUP);
			outprfge(FLT_NONE,usrn);
		}
		else {
...
{
	unsigned __int64 total100, cap100;
	int i;

	total100 = 0;
	cap100 = (unsigned __int64)cap * 100;

	/* ge-next32 divergence: use a 64-bit accumulator instead of 16-bit overflow decomposition. */
	for (i = 0; i < NUMITEMS; ++i)
		total100 += (unsigned __int64)wptr->items[i] * (unsigned long)weight[i];

	if (total100 > cap100)
		return FALSE;

	*tons = (unsigned long)(total100 / 100);
	*rem100 = (unsigned)(total100 % 100);
...
{
	unsigned __int64 total100, cap100, add100;
	unsigned long cap, tons;
	unsigned rem100;
	...
	/* ge-next32 divergence: compare the added weight directly in 64-bit hundredths. */
	total100 = (unsigned __int64)tons * 100 + rem100;
	cap100 = (unsigned __int64)cap * 100;
	add100 = (unsigned __int64)amt * (unsigned long)weight[itm];

	return add100 <= cap100 - total100;
}

unsigned long FUNC cargo_room_for_item(WARSHP *wptr, int itm)
{
	unsigned __int64 total100, cap100, room;
	unsigned long cap, tons, maxqty;
	unsigned rem100;
	...
	maxqty = ULCAP - wptr->items[itm];
	if (weight[itm] == 0)
		return maxqty;

	/* ge-next32 divergence: calculate room once instead of repeatedly scanning in a binary search. */
	total100 = (unsigned __int64)tons * 100 + rem100;
	cap100 = (unsigned __int64)cap * 100;
	room = (cap100 - total100) / (unsigned long)weight[itm];
...
void FUNC set_dislike(WARUSR *wuptr, int facnum, int dislike)
{
	...
	if ((unsigned int)(wuptr->factions[facnum]) + dislike > 255)
		...
		wuptr->factions[facnum] = (byte)(wuptr->factions[facnum] + dislike);
}
...
void FUNC update_scantab(WARSHP *ptr, int usrn)
{
			...
			if (flag != 0) {
				for (i = 0; i < NOSCANTAB; ++i) {
					...
					if (tmp.ship[i].flag == 0) {
						tmp.ship[i].flag = (CHAR)flag;
						tmp.ship[i].shipno = (SHORT)othusn;
	...
	for (i = 0; i < NOSCANTAB; ++i) {
		...
		if (tmp.ship[i].flag == 0) {
			...
			j = tmp.ship[i].shipno;
			wptr = warshpoff(j);
			tmp.ship[i].bearing = (SHORT)cbearing(&ptr->coord, &(wptr->coord), ptr->heading);
			tmp.ship[i].heading = (SHORT)cbearing(&(wptr->coord), &ptr->coord, wptr->heading);
